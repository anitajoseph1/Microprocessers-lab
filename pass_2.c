...
int locctr, start, programLength, len = 0, textStartAddr;
...
if (!optab || !intermediate || !symtab || !output || !length) {//to check whether there is any error in opening the file
...
fprintf(output, "%s\n",objectCode);
if (len + strlen(objectCode) > 60) {//to check whether the text record is full
fprintf(objectcode, "T^%06d^%02X^%s\n", textStartAddr, len / 2, textRecord);
...
fprintf(objectcode, "T^%06d^%02X^%s\n", textStartAddr, len / 2, textRecord); // Ensure the last text record is written
}
fprintf(output, "%d %-7s%-7s%-7s", locctr, label, opcode, operand);
